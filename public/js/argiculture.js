...
$(document).ready(function() {
    const productRegistryContractAddress = '0x167621f90516826A95dEF7711bC28f2758a75693';  // CA 주소
    const productRegistryContractABI =   // ABI 주소
	[
		{
			"constant": false,
			"inputs": [
	...
	async function showTable() {
		...
		if (window.ethereum) // meta mask 설치했냐
			try {
				await window.ethereum.enable();
			} catch (err) {
                return showError("Access to your Ethereum account rejected.");
			}
		if (typeof web3 === 'undefined')  // 없으면 설치해라
		...
		contract.getNumOfProducts(function(err, result) { // smart contract 함수 이름
			if (err)
